    ...
    for UC in UC_TREATMENTS:
        ...
        for T_p in line_limit_scaling:
            ...
            for BA_hurd in BA_hurdle_scaling:
                ...
                path=str(Path.cwd()) + str(Path('/Simulation_folders_EIA/Exp' + str(NN) + UC + '_' + str(T_p) + '_' + str(BA_hurd)))
                ...
                df_hydro = pd.read_csv('BA_data/BA_hydro.csv',header=0,index_col=0)
                ...
                for b in BAs:
                    sample = list(df_full.loc[(df_full['NAME']==b) & (df_full['FuelType'] == 'SUN (hydro)'),'MWMax'])
                    # corrected = [0 if math.isnan(x) else x for x in sample]
                    BA_totals.append(sum(sample))
                ...
                for i in range(0,len(df_full)):
                    ...
                    if str(area) == 'nan':
                        weights.append(0)
                    elif str(df_full.loc[i,'FuelType']) != 'SUN (hydro)':
                        weights.append(0)
                    else:        
                        X = float(df_BA_totals.loc[df_BA_totals['Name']==area,'Total'])
                        W = df_full.loc[i,'MWMax']/X
                ...
                df_S = pd.read_csv('nodal_hydro.csv',header=0)
                buses = list(df_S.columns)
                for n in buses:
                    if sum(df_S[n]) > 0:
                        name = n + '_hydro'
                ...
                #copy other files
                w = 'wrapper' + UC + '_EIA.py'
                milp = 'WECC_MILP' + UC + '.py'
                lp = 'WECC_LP' + UC + '.py'
                
                copy(w,path)
                copy('WECCDataSetup_EIA.py',path)
                
                if UC == '_simple':
                    copy('WECC' + UC + '_EIA.py',path)
                else:          
                    copy(milp,path)
                    copy(lp,path)

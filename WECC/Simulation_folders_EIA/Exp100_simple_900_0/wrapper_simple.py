...
for week in range(1,weeks+1):
    ...
    #load Wind time series data
        for i in K:
            instance.HorizonWind[z,i] = instance.SimWind[z,(week-1)*168+i]
    ...
    for c in instance.component_objects(Constraint, active=True):
        ...
        if str(c) in ['Node_Constraint']:
            for index in cobject:
                 if int(index[1]>0 and index[1]<169):
                     try:
                         duals.append((index[0],index[1]+((week-1)*168), instance.dual[cobject[index]]))
                     except KeyError:
                         duals.append((index[0],index[1]+((week-1)*168),-999))
            ...
            for index in varobject:
                if int(index[1]>0 and index[1]<169):
                    if index[0] in instance.buses:
                        vlt_angle.append((index[0],index[1]+((week-1)*168),varobject[index].value))
            ...
            for index in varobject:
                ...
                if int(index[1]>0 and index[1]<169):
                    ...
                    if index[0] in instance.Gas:
                        ...
                        mwh.append((index[0],'Gas',index[1]+((week-1)*168),varobject[index].value,marginal_cost))   
                    elif index[0] in instance.Coal:
                        marginal_cost = gen_heatrate*fuel_price
                        mwh.append((index[0],'Coal',index[1]+((week-1)*168),varobject[index].value,marginal_cost))  
                    elif index[0] in instance.Oil:
                        marginal_cost = 0
                        mwh.append((index[0],'Oil',index[1]+((week-1)*168),varobject[index].value,marginal_cost))   
                    elif index[0] in instance.Hydro:
                        marginal_cost = 0
                        mwh.append((index[0],'Hydro',index[1]+((week-1)*168),varobject[index].value,marginal_cost)) 
                    elif index[0] in instance.Solar:
                        marginal_cost = 0
                        mwh.append((index[0],'Solar',index[1]+((week-1)*168),varobject[index].value,marginal_cost))
                    elif index[0] in instance.Wind:
                        marginal_cost = 0
                        mwh.append((index[0],'Wind',index[1]+((week-1)*168),varobject[index].value,marginal_cost))                                            
        
        if a=='on':  
            for index in varobject:
                if int(index[1]>0 and index[1]<169):
                    on.append((index[0],index[1]+((week-1)*168),varobject[index].value))

        if a=='switch':
            for index in varobject:
                if int(index[1]>0 and index[1]<169):
                    switch.append((index[0],index[1]+((week-1)*168),varobject[index].value))
                    
        if a=='S':    
            for index in varobject:
                if index[0] in instance.buses:
                        slack.append((index[0],index[1]+((week-1)*168),varobject[index].value))

        if a=='Flow':    
            for index in varobject:
                if int(index[1]>0 and index[1]<169):
                    flow.append((index[0],index[1]+((week-1)*168),varobject[index].value))                                            

        # if a=='srsv':    
        #     for index in varobject:
        #         if int(index[1]>0 and index[1]<169):
        #             srsv.append((index[0],index[1]+((week-1)*168),varobject[index].value))

        # if a=='nrsv':    
        #     for index in varobject:
        #         if int(index[1]>0 and index[1]<169):
        #             nrsv.append((index[0],index[1]+((week-1)*168),varobject[index].value))
        
                                
        for j in instance.Dispatchable:
            if instance.mwh[j,168].value <=0 and instance.mwh[j,168].value>= -0.0001:
                newval_1=0
            else:
                newval_1=instance.mwh[j,168].value
